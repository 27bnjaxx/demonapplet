...
import com.sun.jcclassic.samples.samplelibrary.JavaLoyaltyInterface;

/**
 * This class is intended to demonstrate how an electronic cash application
 * might be developed using Java Card
 * <p>
 * See <em>Java Card(TM) Reference Implementation User's Guide</em> for
 * details.
 * 
 * @author Vadim Temkin, Modified to include cryptographic signatures by M.
 *         Saltz
 */
public class JavaPurseCrypto extends Applet {
     ...
     *
     */
    final static byte VERIFY = (byte) 0x20; // INS value for ISO 7816-4 VERIFY
    // command
    final static byte READ = (byte) 0xB2; // INS value for ISO 7816-4 READ
    // RECORD command
    final static byte INITIALIZE_TRANSACTION = (byte) 0x20; // INS byte for
    // Initialize
    // Transaction
    final static byte COMPLETE_TRANSACTION = (byte) 0x22; // INS byte for
    // Complete
    // Transaction
    final static byte INITIALIZE_UPDATE = (byte) 0x24; // INS byte for
    // Initialize Parameter
    // Update
    final static byte COMPLETE_UPDATE = (byte) 0x26; // INS byte for Complete
    // Parameter Update
    final static byte CREDIT = (byte) 0x01; // P1 byte for Credit in Initialize
    // Transaction
    final static byte DEBIT = (byte) 0x02; // P1 byte for Debit in Initialize
    // Transaction
    final static byte MASTER_PIN = (byte) 0x81; // P2 byte for Master PIN in
    // Verify
    final static byte USER_PIN = (byte) 0x82; // P2 byte for User PIN in
    // Verify

    final static short MAX_MAC_DATA = (short) 0x001E; // maximimum length of
    // data used in MAC
    final static short SW_CREDIT_TOO_HIGH = (short) 0x9101;// SW bytes for
    // Credit Too High
    // condition
    final static short SW_NOT_ENOUGH_FUNDS = (short) 0x9102;// SW bytes for Not
    // Enough Funds
    // condition
    final static short SW_AMOUNT_TOO_HIGH = (short) 0x9103;// SW bytes for
    // Amount Too High
    // condition
    final static short SW_COMMAND_OUT_OF_SEQUENCE = (short) 0x9104;// SW bytes
    // for
    // Command
    // out of
    // Sequence
    final static short SW_WRONG_SIGNATURE = (short) 0x9105;// SW bytes for
    // Wrong Signature
    // condition
    final static short SW_PIN_FAILED = (short) 0x69C0;// SW bytes for PIN
    // Failed condition
    final static short SW_SUCCESS = (short) 0x9000;// SW bytes for success,
    // used in MAC.
    ...
    final static byte CAD_ID_OFFSET = 7; // Offset for CAD ID in Process
    ...
    final static short SHORT_LENGTH = 2; // Length of a short value for
    // offset computations
    final static short START = 0; // For offset computations
    final static short SIGNATURE_LENGTH = 8; // Length of signatures
    final static short MAX_LOYALTY = 4; // Max number of loyalty applets

    // transientShorts array indices
    final static byte TN_IX = 0;
    final static byte NEW_BALANCE_IX = TN_IX + 1;
    final static byte CURRENT_BALANCE_IX = NEW_BALANCE_IX + 1;
    final static byte AMOUNT_IX = CURRENT_BALANCE_IX + 1;
    final static byte TRANSACTION_TYPE_IX = AMOUNT_IX + 1;
    final static byte SELECTED_FILE_IX = TRANSACTION_TYPE_IX + 1;
    final static byte NUM_TRANSIENT_SHORTS = SELECTED_FILE_IX + 1;

    // transientBools array indices
    final static byte TRANSACTION_INITIALIZED = 0;
    final static byte UPDATE_INITIALIZED = TRANSACTION_INITIALIZED + 1;
    final static byte NUM_TRANSIENT_BOOLS = UPDATE_INITIALIZED + 1;
    ...
    private static byte[] FCI_PROPERIETARY = { (byte) 0xA5, // tag
            (byte) 0x01, // length
            (byte) 0x42 }; // value
    ...
    private short TN;
    private short PUN;
    private boolean isPersonalized; // set to true when Master PIN is updated
    // first time
    /*
    ...
    final static short TLV_OFFSET = 13; // Offset of TLV in Complete Parameter
    // Update
     ...
     */
    byte[] keyData = { (byte) 0x11, (byte) 0x33, (byte) 0x22, (byte) 0x05, (byte) 0x44, (byte) 0x77, (byte) 0x06,
            (byte) 0x08 };
    ...
    */
    ...
    protected JavaPurseCrypto(byte[] bArray, short bOffset, byte bLength) {
        ...
        transientShorts = JCSystem.makeTransientShortArray(NUM_TRANSIENT_SHORTS, JCSystem.CLEAR_ON_DESELECT);
        transientBools = JCSystem.makeTransientBooleanArray(NUM_TRANSIENT_BOOLS, JCSystem.CLEAR_ON_DESELECT);

        CAD_ID_array = JCSystem.makeTransientByteArray((short) 4, JCSystem.CLEAR_ON_DESELECT);

        // initialize crypto key/signature objects and work array.
        byteArray8 = JCSystem.makeTransientByteArray((short) 8, JCSystem.CLEAR_ON_DESELECT);
        ...
        // There is only one try - it's not supposed to be done by human
        // operator
        userPIN = new OwnerPIN((byte) 5, (byte) 8);

        parametersFile = new ParametersFile(PARAMETERS_RECORD_NUMBER);

        transactionLogFile = new CyclicFile(TRANSACTION_RECORD_NUMBER, TRANSACTION_RECORD_LENGTH);
        ...
        if (!apdu.isISOInterindustryCLA()) {
            switch (buffer[ISO7816.OFFSET_INS]) {
                case INITIALIZE_TRANSACTION:
                    processInitializeTransaction(apdu);
                    break;
                case COMPLETE_TRANSACTION:
                    processCompleteTransaction(apdu);
                    break;
                case INITIALIZE_UPDATE:
                    processInitializeUpdate(apdu);
                    break;
                case COMPLETE_UPDATE:
                    processCompleteUpdate(apdu);
                    break;
                default:
    ...
    private void processSelectPurse(APDU apdu) {
        ...
        // different AID values. This method returns the FCI here identifying
        // its
        // particular instance.
        //
        ...
        offset = Util.arrayCopyNonAtomic(FCI_PROPERIETARY, (short) 0, buffer, offset, (short) FCI_PROPERIETARY.length);
     ...
     * <p>
     * See <em>Java Card 2.1 Reference Implementation User's Guide</em> for
     * details.
     * 
     * @param apdu
     *            APDU object
    ...
    private void processInitializeTransaction(APDU apdu) {
        ...
        short sigLength = sig.sign(buffer, START, (short) (offset - START), byteArray8, (short) 0);
    ...
    private void processCompleteTransaction(APDU apdu) {
        ...
        Util.arrayCopyNonAtomic(buffer, (short) (offset + SIGNATURE_LENGTH), MAC_buffer, (byte) 5, DATETIME_LENGTH);
        deskey.setKey(keyData, (short) 0);
        sig.init(deskey, Signature.MODE_VERIFY);
        boolean signatureOK = sig.verify(MAC_buffer, (short) 0, (short) 10, byteArray8, START, SIGNATURE_LENGTH);
        ...
        short balanceOffset = offset = Util.arrayCopyNonAtomic(buffer, (short) (ISO7816.OFFSET_CDATA + 8), buffer,
        ...
        if (!signatureOK) {
            ...
            // otherwise transactionLog is recorded the same way as in
            // successful transaction
            offset = Util.setShort(buffer, offset, transientShorts[CURRENT_BALANCE_IX]); // old
            // balance
            ...
            TN = newTN;
            Util.arrayCopy(buffer, START, theRecord, START, TRANSACTION_RECORD_LENGTH);
        ...
        // Do loyalty work
        // We have all the information in the buffer, the loyalty applets
        // shouldn't
        // know transaction number and the balance of purse - so we zero out
        // these
        // fields first
        ...
        deskey.setKey(keyData, (short) 0);
        sig.init(deskey, Signature.MODE_SIGN);
        short sigLength = sig.sign(MAC_buffer, (short) 0, (short) 4, byteArray8, (short) 0);
     ...
     * 
     * <p>
     * <em>NOTE:</em> In this sample implementation we assume that all the
     * Parameter Updates are performed in somewhat secured facility and
     * therefore the tearing of the card is not an issue. That's why we don't do
     * any transactional protection while processing Initialize and Complete
    ...
    private void processInitializeUpdate(APDU apdu) {
        ...
        // Because this is a case 2 command (outgoing data only), the contents
        // of P3
        // are undefined. In T=0, P3 is Le. In T=1, P3 is Lc. Therefore, we
        // don't
    ...
    private void processCompleteUpdate(APDU apdu) {
        ...
        short offset = Util.arrayCopyNonAtomic(buffer, (short) 0, MAC_buffer, (byte) 0, messageLength);
        offset = Util.arrayCopyNonAtomic(buffer, offset, byteArray8, (byte) 0, SIGNATURE_LENGTH);
        // verify signature if in coming apdu
        deskey.setKey(keyData, (short) 0);
        sig.init(deskey, Signature.MODE_VERIFY);
        boolean signatureOK = sig.verify(MAC_buffer, (short) 0, messageLength, byteArray8, START, SIGNATURE_LENGTH);
        ...
        // Admittedly, this is a poor MAC, but this message only returns the
        // status
        // (what is MACed here) and the MAC itself. A real application should
        // return a
        // little more information.
        offset = Util.setShort(MAC_buffer, START, SW_SUCCESS);
        deskey.setKey(keyData, (short) 0);
        sig.init(deskey, Signature.MODE_SIGN);
        short sigLength = sig.sign(MAC_buffer, (short) 0, (short) 2, byteArray8, (short) 0);

        // send R-APDU
        // offset = Util.arrayCopyNonAtomic(byteArray8, START, buffer, offset,
        // SIGNATURE_LENGTH);
        ...
        switch (pinType) {
            case MASTER_PIN:
                if (!masterPIN.check(buffer, ISO7816.OFFSET_CDATA, pinLength)) {
                    ...
                    // The last nibble of return code is number of remaining
                    // tries
     ...
     * 
     * <p>
     * <em>NOTE:</em> With some values of maxBalance and maxAmount the logic
     * in this method might become somewhat unrealistic. It's the result of
     * using short arithmetic on values which might be too big to fit in short
     * variables.
     * 
    ...
    private short checkTransactionValues(byte transactionType, short amount) {
        ...
        switch (transactionType) {
            case CREDIT: {
                ...
                if (newBalance > maxBalance || newBalance < 0) {
            ...
            case DEBIT: {
                if (amount > maxAmount) {
                    ISOException.throwIt(SW_AMOUNT_TOO_HIGH);
                }
                newBalance = (short) (currentBalance - amount);
                transientShorts[NEW_BALANCE_IX] = newBalance;
                if (newBalance < 0) {
                    ISOException.throwIt(SW_NOT_ENOUGH_FUNDS);
                }
                break;
            }
            default:
     ...
     *            APDU object
     * @param PIN
     *            OwnerPIN object (masterPIN or userPIN)
     */
    ...
    /**
     * Set JavaPurse in personalized state. It happens only once - after the
     * first update of masterPIN
     */

    private void setIsPersonalized() {
        if (!isPersonalized) {
            isPersonalized = true;// happens only once
        }
    }
    ...
    /**
     * Updates values of maximum balance or maximum amount for transaction in
     * the balancesRecord.
     * 
     * @param apdu
     *            APDU object
     * @param offset
     *            the offset in balancesRecord to be updated
     */
    ...
    private void updateParametersFile(APDU apdu) {
        byte[] buffer = apdu.getBuffer();
        byte recordNumber = parametersFile.findRecord(buffer[TLV_OFFSET]);// match
        // tag
        ...
        if (recordNumber == (byte) 0) {
            ...
            Util.arrayCopyNonAtomic(buffer, TLV_OFFSET, newRecord, START, (short) (buffer[TLV_OFFSET + 1] + 2));
            parametersFile.addRecord(newRecord);
        } else {
            byte[] theRecord = parametersFile.getRecord(recordNumber);
            Util.arrayCopyNonAtomic(buffer, TLV_OFFSET, theRecord, START, (short) (buffer[TLV_OFFSET + 1] + 2));
        }
    }

    /**
     * Selects file by FID according to ISO7816-4. This implementation doesn't
     * support all variations of SELECT command in the standard, but provides
     * reasonable subset for selection by FID (P1==2).
     * 
     * @param apdu
     *            APDU object
     */
    ...
    /**
     * Reads a record by the record number or reads the first occurrence of the
     * record by the record identifier. This implementation doesn't support all
     * variations of READ RECORD command in the ISO 7816-4 standard, but
     * provides reasonable subset of it. It is here to demonstrate that even
     * without "built-in" support for ISO 7816 File System an Applet can have
     * the behavior prescribed by the standard.
     * 
     * @param apdu
     *            APDU object
     */
    ...
    /**
     * Updates loyalty program. It takes standard TLV record for Parameter
     * Update, interprets first two bytes as loyaltyCAD (to be compared with
     * first two bytes of CAD ID in a transaction), the rest of record as AID
     * for loyalty applet. In case of successful retrieval of Shareable
     * Interface Object with this AID it is stored as an element of array, and
     * method grantPoints of this loyalty applet will be called in transaction
     * to grant loyalty points. If SIO is not returned, or loyaltyCAD in
     * parameter is 0 the corresponding elements in loyaltyCAD and loyaltySIO
     * arrays are cleared. The Parameters File is updated.
     * 
     * @param apdu
     *            APDU object
     * @param loyaltyIndex
     *            index to loyaltyCAD and loyaltySIO arrays
    ...
    private void updateLoyaltyProgram(APDU apdu, byte loyaltyIndex) {
        ...
        if (loyaltyCAD[loyaltyIndex] != (short) 0) {
            ...
            AID loyaltyAID = JCSystem.lookupAID(buffer, (short) (TLV_OFFSET + 4), (byte) (buffer[TLV_OFFSET + 1] - 2));
            if (loyaltyAID != null) {
                loyaltySIO[loyaltyIndex] = (JavaLoyaltyInterface) JCSystem.getAppletShareableInterfaceObject(
                        loyaltyAID, (byte) 0);
        ...
        if (loyaltyCAD[loyaltyIndex] == (short) 0) {
            ...
            Util.arrayFillNonAtomic(buffer, (short) (TLV_OFFSET + 2), (short) (buffer.length - TLV_OFFSET - 2),
                    (byte) 0);
        }
        updateParametersFile(apdu);
    }
