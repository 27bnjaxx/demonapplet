...
package com.sun.jcclassic.samples.javaloyalty;
...
/**
 * Minimalistic loyalty applet. JavaPurse uses grantPoints method implementing
 * JavaLoyaltyInterface to grant loyalty points. JavaLoyalty can process only
 * two command APDUs: READ_BALANCE and RESET_BALANCE.
 *
 * @author Vadim Temkin
 */
public class JavaLoyalty extends Applet implements JavaLoyaltyInterface {
    ...
    final static byte RESET_BALANCE = (byte) 0x22; // INS byte for Reset
    // Balance
    final static byte CREDIT = (byte) 0x01; // byte for Credit in grantPoints
    // buffer
    final static byte DEBIT = (byte) 0x02; // byte for Debit in grantPoints
    // buffer
    final static short TRANSACTION_TYPE_OFFSET = 2;
    final static short TRANSACTION_AMOUNT_OFFSET = 3;
    final static short SCALE = (short) 100; // 1 Loyalty Point per $1 (100c)
    // debit
    final static short BALANCE_MAX = (short) 30000; // One won't get free flight
    // from SFO to LAX
