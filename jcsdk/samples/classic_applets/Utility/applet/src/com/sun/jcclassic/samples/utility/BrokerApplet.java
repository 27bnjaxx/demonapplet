...
package com.sun.jcclassic.samples.utility;
...
public class BrokerApplet extends Applet {
     ...
     */
    final static byte[] INITIAL_ACCOUNT_BALANCE = { (byte) 0x01, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00,
            (byte) 0x00 };
    /**
     ...
     */
    private static final byte[] dummySignature = { (byte) 0x88, (byte) 0x88, (byte) 0x88, (byte) 0x88, (byte) 0x88,
    ...
    BrokerApplet(byte[] bArray, short bOffset, byte bLength) {
        ...
        pin.update(bArray, bOffset, (paramLen));

        // initialize account balance to 100,000.00
        accountBalance = new BigNumber((byte) 8);
        accountBalance.init(INITIAL_ACCOUNT_BALANCE, (byte) 0, (byte) INITIAL_ACCOUNT_BALANCE.length,
        ...
        portfolio = (ConstructedBERTLV) BERTLV.getInstance(scratchSpace, (short) 0, (short) 2);
    }
    ...
    public void process(APDU apdu) throws ISOException {
        ...
        // get balance command
        if (buffer[ISO7816.OFFSET_INS] != GET_COMPLETE_PORTFOLIO && buffer[ISO7816.OFFSET_INS] != GET_BALANCE) {
    ...
    /**
     * Return following information on all the stocks - Primitive TLV for stock
     * symbol - Primitive TLV for number of stocks we currently have -
     * Constructed TLV for last trade containing following - Primitive TLV for
     * number of stocks - Primitive TLV for price
     *
     * @param buffer
     *            is the input APDU buffer
     ...
     * Output: Constructed BER TLV containing the following: - Primitive TLV for
     * stock symbol - Primitive TLV for number of stocks we currently have -
     * Constructed TLV for last trade containing following - Primitive TLV for
     * number of stocks - Primitive TLV for price
     *
     * @param buffer
     ...
     *
     * @param buffer
     *            is the byte array containing the stock symbol
     * @param offset
     *            is the offset in the byte array for stock symbol
     * @return ConstructedBERTLV for the stock information if stock is found. If
     *         the stock is not found ISOException is thrown
     */
    private ConstructedBERTLV findStock(byte[] buffer, short offset) {
        // We go through all the stocks that we have to find the
        // one we're looking for
        ConstructedBERTLV stockInfo = (ConstructedBERTLV) portfolio.find(stockInfoTag);

        while (stockInfo != null) {
            PrimitiveBERTLV symbol = (PrimitiveBERTLV) stockInfo.find(symbolTag);
            symbol.getValue(scratchSpace, (short) 0);
            if (Util.arrayCompare(buffer, offset, scratchSpace, (short) 0, STOCK_SYMBOL_LENGTH) == 0) {
                return stockInfo;
            }
            stockInfo = (ConstructedBERTLV) portfolio.findNext(stockInfoTag, stockInfo, (short) 1);
        }
        return null;
    }
    ...
    /**
     * Performs the following steps to assist in buying a stock - Check
     * available funds - Create a "signed" request for the broker
     *
     * @param buffer
     *            is the input APDU buffer
     * @return size of response data
    ...
    private byte genStockPurchaseRequest(byte[] buffer) {
        ...
        offset = Util.arrayCopyNonAtomic(buffer, ISO7816.OFFSET_CDATA, scratchSpace, offset, STOCK_SYMBOL_LENGTH);
     ...
     *
     * @param buffer
     *            is the input buffer containing the number of stocks we want to
    ...
    /**
     * Returns stock a sell stock request to the client if we hold enough
     * stocks. Input: Stock symbol, the number of stocks to sell and desired
     * price Output: Constructed BER TLV containing the following: - Primitive
     * TLV for stock symbol - Primitive TLV for desired stock price - Primitive
     * TLV for number of stocks to sell - Primitive TLV for signature
     *
     * @param buffer
     *            is the input APDU buffer
     * @return size of response data
    ...
    private byte genSellStockRequest(byte[] buffer) {
        ...
        PrimitiveBERTLV numStocksTLV = (PrimitiveBERTLV) stockInfo.find(numStocksTag);
        ...
        // output the stock symbol TLV
        PrimitiveBERTLV stockSymbol = (PrimitiveBERTLV) stockInfo.find(symbolTag);
    ...
    /**
     * This method completes the stock sell or purchase request for broker. In
     * this method we add the following primitive TLVs to the broker request
     * constructed TLV Primitive TLV for desired stock price Primitive TLV for
     * number of stocks to sell Primitive TLV for signature
     *
     * @param buffer
     *            is the input APDU buffer
     * @return size of response data
     */
    private void completeBrokerRequest(byte[] buffer, short priceToSellAt, int numStocksToSell) {
        short offset = 0;
        ...
        Util.arrayCopyNonAtomic(dummySignature, (short) 0, scratchSpace, offset, (byte) 8);
        ConstructedBERTLV.append(scratchSpace, (short) 0, buffer, (short) 0);
    }

    /**
     * Verifies the signature of broker and if the signature is verified update
     * the portfolio including stocks and available balance
     *
     * @param buffer
     *            is the input APDU buffer
     */
    private void updatePortfolio(byte[] buffer) {
        ...
        short symbolTLVOffset = ConstructedBERTLV.find(buffer, offset, scratchSpace, (short) 0);

        numStocksTag.toBytes(scratchSpace, (short) 0);
        short numStocksTLVOffset = ConstructedBERTLV.find(buffer, offset, scratchSpace, (short) 0);
        short numStocksValueOffset = PrimitiveBERTLV.getValueOffset(buffer, numStocksTLVOffset);
        // get the stock price
        ...
        ConstructedBERTLV stockInfo = findStock(buffer, PrimitiveBERTLV.getValueOffset(buffer, symbolTLVOffset));
        ...
        if (stockInfo == null) {
            ...
            stockInfo = createNewStockTLV(buffer, symbolTLVOffset, numStocksTLVOffset, priceTLVOffset);
            portfolio.append(stockInfo);
            updateAccountBalance(buffer, numStocksValueOffset, priceValueOffset, isSellConfirmation);
            return;
        }
        ...
        if (!isSellConfirmation) {
            tempBigNum.add(buffer, numStocksValueOffset, (short) 4, BigNumber.FORMAT_HEX);
        } else {
            tempBigNum.subtract(buffer, numStocksValueOffset, (short) 4, BigNumber.FORMAT_HEX);
            // if stocks now number 0, we remove this TLV
            Util.arrayFillNonAtomic(scratchSpace, (short) 0, (short) 1, (byte) 0);
            if (tempBigNum.compareTo(scratchSpace, (short) 0, (short) 1, BigNumber.FORMAT_HEX) == 0) {
                portfolio.delete(stockInfo, (short) 1);
                return;
            }
        }
        tempBigNum.toBytes(scratchSpace, (short) 0, (short) 4, BigNumber.FORMAT_HEX);
        numStocksTLV.replaceValue(scratchSpace, (short) 0, (short) 4);

        // update last trade information
        ConstructedBERTLV lastTradeTLV = (ConstructedBERTLV) stockInfo.find(lastTradeTag);
     ...
     * @param buffer
     *            is the input buffer
     * @param numStocksOffset
     *            is the offset in the buffer where number of stocks information
     *            is at
     * @param priceOffset
     *            is the offset in the buffer where stock price information is
     *            at
     * @param sold
     *            is the boolean indicating the if the stock was bought or sold
     */
    void updateAccountBalance(byte[] buffer, short numStocksOffset, short priceOffset, boolean sold) {
        ...
        if (sold) {
            ...
            accountBalance.add(scratchSpace, (short) 0, (short) 8, BigNumber.FORMAT_HEX);
        } else {
     ...
     *            is the input APDU buffer
     * @param TLVOffset
     *            is the offset within the buffer where the TLV for the broker
     *            signature starts
     */
    void verifyBrokerSignature(byte[] buffer, short TLVOffset) {
        // get the stock price
        signatureTag.toBytes(scratchSpace, (short) 0);
        short sigTLVOffset = ConstructedBERTLV.find(buffer, TLVOffset, scratchSpace, (short) 0);
        short sigValueOffset = PrimitiveBERTLV.getValueOffset(buffer, sigTLVOffset);
        if (Util.arrayCompare(buffer, sigValueOffset, dummySignature, (short) 0, (byte) dummySignature.length) != 0) {
            ISOException.throwIt(INVALID_BROKER_SIGNATURE);
        }
    }

    /**
     * As a rsult of a stock that is bought, we create a new TLV to hold the
     * information regarding the newly bought stock
     *
     * @param buffer
     *            contains the new stock information
     * @param symbolOffset
     *            is the offset within buffer where the new stock symbol is
     *            present
     * @param numOffset
     *            is the offset within the buffer where the information
     *            regarding number of stocks is at
     * @param priceOffset
     *            is the offset within the buffer where the information
     *            regarding price at which the stock was bought is at
     * @return newly created constructed BERTLV object that is eventually added
     *         to the portfolio.
     */
    private ConstructedBERTLV createNewStockTLV(byte buffer[], short symbolOffset, short numOffset, short priceOffset) {
        ...
        ConstructedBERTLV stockInfo = (ConstructedBERTLV) BERTLV.getInstance(scratchSpace, (short) 0, (short) 2);
        // create the stock symbol TLV
        offset = symbolTag.toBytes(scratchSpace, (short) 0);
        PrimitiveBERTLV symbolTLV = (PrimitiveBERTLV) BERTLV.getInstance(buffer, symbolOffset, (short) 7);
        // append to the stockInfo TLV
        stockInfo.append(symbolTLV);

        // clean up the scratch space for number TLV
        PrimitiveBERTLV numStocksTLV = (PrimitiveBERTLV) BERTLV.getInstance(buffer, numOffset, (short) 6);
        // append to the stockInfo TLV
        ...
        ConstructedBERTLV lastTradeInfo = (ConstructedBERTLV) BERTLV.getInstance(scratchSpace, (short) 0, (short) 2);

        PrimitiveBERTLV LTradeNumStocksTLV = (PrimitiveBERTLV) BERTLV.getInstance(buffer, numOffset, (short) 6);
        lastTradeInfo.append(LTradeNumStocksTLV);

        PrimitiveBERTLV priceTLV = (PrimitiveBERTLV) BERTLV.getInstance(buffer, priceOffset, (short) 4);
