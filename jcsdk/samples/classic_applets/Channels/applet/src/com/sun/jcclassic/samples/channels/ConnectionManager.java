...
package com.sun.jcclassic.samples.channels;
...
/**
 * This applet keeps track of the network connection for a fictional wireless
 * device. Every time unit the device is being actively used on the network will
 * result in decreasing credits available to the user. If the user changes
 * areas, different charge rates applies. If the user runs out of credits, then
 * the connection is terminated.
 */

public class ConnectionManager extends Applet implements MultiSelectable, ExtendedLength {
    ...
    private void validateKey(APDU apdu) {
        ...
        short testKey = (short) ((short) (buffer[ISO7816.OFFSET_EXT_CDATA] << (short) 8) | (short) (buffer[ISO7816.OFFSET_EXT_CDATA + 3] & 0x00FF));
        if (testKey != (short) 3) {
    ...
    private void timeTick(APDU apdu) {
        ...
        // get area code
        short newAreaCode = (short) ((short) (buffer[ISO7816.OFFSET_CDATA] << (short) 8) | (short) (buffer[ISO7816.OFFSET_CDATA + 1] & 0x00FF));
